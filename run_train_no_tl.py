...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=30,
        type=int,
        help="dimensionality of meta data embedding",
    )
    parser.add_argument(
        "--random_mask",
        action="store_true",
        help="randomly bask input for base level masking",
    )
    parser.add_argument(
        "--mlm_prob",
        default=0.1,
    ...
    parser.add_argument(
        ...
        default='none',
        type=str,
        help="aggregation applied to the losses computed after multiplying individual losses with mask specific weights",
    )
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        if args.test_file is not None:
            data_files["test"] = os.path.abspath(args.test_file)
    ...
    if args.model_name_or_path:
        ...
        if args.entity_masking:
            model = joint_model.JointModel(model=model,
                                           config=config,
    ...
    if args.entity_masking:
        ...
        if "test" in raw_datasets:
            test_dataset = custom_mask.customMask(test_dataset,
                                                  tokenizer=tokenizer,
                                                  labels_mapping=model.config.id2label,
                                                  custom_mask=True,
                                                  random_mask=args.random_mask,
    ...
    if args.entity_masking:
        #fetch weights for each token within each sample
        if args.test_file:
            loaders = [train_dataloader, eval_dataloader, test_dataloader]
        else:
            loaders = [train_dataloader, eval_dataloader]
        ...
        eval_weights = utils.compute_mask_specific_weights(ev_tracked_data_ids,
        ...
        if args.test_file:
            te_tracked_data_ids = tracked_dataset_ids[2]
            te_weights = utils.compute_mask_specific_weights(te_tracked_data_ids,
                                                             batch_size=args.per_device_eval_batch_size,
                                                             seq_len=args.max_length,
                                                             weight_matrix=weight_matrix).to(device)

    epoch_batch_exact_match_evaluation = []
    tracking_metrics = {'loss': [], 'exact_match_score': [], 'F1': [], 'perplexity':[]}
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        model.eval()
        samples_seen = 0
        batch_exact_match_evaluation, losses = [], []
        ...
        batch_exact_match_score = np.mean([i[-1] for i in batch_exact_match_evaluation if i[0] > 0])
        ...
        if args.with_tracking:
            ...
            accelerator.print(f"\nepoch {epoch}\n"
                              ...
                              f"Results {eval_metric}\n"
                              f"Exact match score {batch_exact_match_score}\n"
    ...
    if args.test_file:
        ...
        if args.with_tracking:
            accelerator.print(f"Test set Results\n{test_metric}\n"
                              f"Exact match score {test_exact_match_score}\n")
